...
/* 显示页面的action列表 */
$ui_arr = array('register', 'login', 'profile', 'order_list', 'cad_download','order_detail', 'address_list', 'collection_list', 'order_print',
...
/* 个人资料页面 */ elseif ($action == 'profile') {
    ...
    foreach ($extend_info_list AS $key => $val) {
        switch ($val['id']) {
            case 1: $extend_info_list[$key]['content'] = $user_info['msn'];
                break;
            case 2: $extend_info_list[$key]['content'] = $user_info['qq'];
                break;
            case 3: $extend_info_list[$key]['content'] = $user_info['office_phone'];
                break;
            case 4: $extend_info_list[$key]['content'] = $user_info['home_phone'];
                break;
            case 5: $extend_info_list[$key]['content'] = $user_info['mobile_phone'];
                break;
...
/* 下载zip文件 */ elseif ($action == 'cad_download') {
    $cat_id = isset($_GET['cat_id']) ? intval($_GET['cat_id']) : 0;
    if (!$cat_id) {
        show_message("分类不存在");
    }
    $cat_info = $GLOBALS['db']->getRow('SELECT * FROM ' . $GLOBALS['ecs']->table('category') .
            " WHERE cat_id = '$cat_id'");
    if (!$cat_info['cad_down']) {
        show_message("该分类下不存在打包文件！");
    }
    $filename = 'data/cad/' . $cat_info['cad_down'];
    if (file_exists($filename)) {
        ...
        show_message("该分类下文件不可读！");
    }
}
/* 打印订单详情 */ elseif ($action == 'order_print') {
    include_once(ROOT_PATH . 'includes/order_pdf.php');
    include_once(ROOT_PATH . 'includes/lib_payment.php');
    include_once(ROOT_PATH . 'includes/lib_order.php');
    include_once(ROOT_PATH . 'includes/lib_clips.php');
    include_once(ROOT_PATH . 'includes/lib_transaction.php');
...
/* ajax 发送验证邮件 */ elseif ($action == 'send_hash_mail') {
    include_once(ROOT_PATH . 'includes/cls_json.php');
    include_once(ROOT_PATH . 'includes/lib_passport.php');
    $json = new JSON();
