...
if (!$smarty->is_cached('goods.dwt', $cache_id)) {
    ...
    if ($goods === false) {
        ...
        $voulme_list = array();
        $voulme_list[0]['number1'] = "1-" . $volume_price_list[0]['number'];
        $voulme_list[0]['format_price'] = $goods['shop_price_formated'];
        if ($volume_price_list) {
            foreach ($volume_price_list as $kk => $vv) {
                $voulme_list[$kk + 1] = $vv;
                if ($kk == (count($volume_price_list) - 1)) {
                    $voulme_list[$kk + 1]['number1'] = $vv['number'] . "-";
                } else {
                    $voulme_list[$kk + 1]['number1'] = $vv['number'] . '-' . $volume_price_list[$kk + 1]['number'];
                }
            }
        }
