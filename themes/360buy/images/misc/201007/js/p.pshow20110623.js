...
getCommentUrl = "".format(skuid);
    $.getJSONP(getCommentUrl, GetCommentSummaryBySkuId);
})();
function GetCommentSummaryBySkuId(result) { if (!result) return; var domobj = $("#star" + result.SkuId); if (!domobj.html()) { domobj = $("div[id^='star']") } domobj.html("<div class=\"star sa" + result.AverageScore + "\"></div><a href=\"./" + result.SkuId + ".html#comment\" class=\"num-comment\">(已有" + result.CommentCount + "人评价)</a>") } 
/*fitting*/
var accessory={settings:{element:"#recommend",elementTab:"#tab-fitting",elementTabcon:"#fitting",selementTab:"#stab-fitting",selementTabcon:"#stabcon-fitting",elementAmount:"#fitting-amount",elementBuy:"#fitting-buy",elementPrice:"#fitting-price",elementSaving:"#fitting-saving",skuid:$("#name").attr("PshowSkuid"),tokey:$("#skuidkey").text(),amount:0,price:0,saving:0,fittingServUrl:"images/360buy/SOAService/RecommendFittings.ashx?skuid={0}&Tokey={1}&callback=?",addSkusUrl:"../../jd2008/purchase/OrderFlowService.aspx@action=AddSkus&wids=",template:"<ul class=\"tab-sub\" id=\"stab-fitting\"><li class=\"fore curr\" onclick=\"accessory.showGoods(this,0)\">全部配件</li>{for type in fittingType}<li onclick=\"accessory.showGoods(this,${type.sort},${type.number})\">${type.name}(${type.number})</li>{/for}</ul><div class=\"tabcon-sub\" id=\"stabcon-fitting\"><div class=\"master\"><div class=\"p-img\"><a target=\"_blank\" href=\"${master.skuid}.html\"><img height=\"100\" width=\"100\" src=\"${master.pic}\"></a></div><div class=\"p-name\"><a target=\"_blank\" href=\"${master.skuid}.html\">${master.name}</a></div><div class=\"icon-add\"></div></div><div class=\"suits\"><ul class=\"list\">{for fitting in fittings}<li data=\"${fitting.sort}\"><div class=\"p-img\"><a target=\"_blank\" href=\"${fitting.skuid}.html\"><img height=\"100\" width=\"100\" alt=\"${fitting.name}\" src=\"${fitting.pic}\" /></a></div><div class=\"p-name\"><a target=\"_blank\" href=\"${fitting.skuid}.html\">${fitting.name}</a></div><div class=\"choose\"><input type=\"checkbox\" onclick=\"accessory.calculate(this,${fitting.skuid},${fitting.price},${fitting.discount})\"/><span class=\"p-price\"><strong>￥${fitting.price}</strong></span></div></li>{/for}</ul></div><div class=\"infos\"><div class=\"selected\">已选择<span id=\"fitting-amount\">0</span>个配件</div><div class=\"p-price\">搭配价：<strong id=\"fitting-price\">￥${master.price}</strong></div>{if master.discount>0}<div class=\"p-saving\">获得优惠：<span id=\"fitting-saving\">￥${master.discount}</span></div>{else}<div class=\"p-saving hide\">获得优惠：<span id=\"fitting-saving\">￥${master.discount}</span></div>{/if}<div class=\"btns\"><a href=\"../../jd2008/purchase/OrderFlowService.aspx@action=AddSkus&wids=${master.skuid}\" class=\"btn-buy\" id=\"fitting-buy\">立即购买</a></div></div></div>"}};accessory.setWidth=function(a,b){var element=a.find(".suits").eq(0);var w=screen.width,x=(w>=1280)?158:140,y=(w>=1280)?632:420,z=(!b)?element.find("li").length:b;element.css({"overflow-x":(x*z>y)?"scroll":"hidden"});element.find("ul").eq(0).css({"width":x*z})};accessory.showGoods=function(a,b,c){var settings=this.settings;$(settings.selementTab).find("li").each(function(){$(this).removeClass("curr")});$(a).addClass("curr");$(settings.selementTabcon).find("li").each(function(){if($(this).attr("data")==b||b==0){$(this).show()}else{$(this).hide()}});this.setWidth($(settings.element),c)};accessory.calculate=function(a,b,c,d){var settings=this.settings,wids=$(settings.elementBuy).attr("href").match(/&wids=([0-9,]{1,})/)[1].split(",");if(a.checked){settings.amount++;settings.price=parseFloat(settings.price)+parseFloat(c);settings.saving=parseFloat(settings.saving)+parseFloat(d);if($.inArray(b,wids)==-1){wids.push(b)}}else{settings.amount--;settings.price=parseFloat(settings.price)-parseFloat(c);settings.saving=parseFloat(settings.saving)-parseFloat(d);$.each(wids,function(i){if(b==wids[i]){wids.splice(i,1);return}})}settings.price=settings.price.toFixed(2);settings.saving=settings.saving.toFixed(2);if(settings.saving>0){$(settings.elementSaving).parent().show()}else{$(settings.elementSaving).parent().hide()}$(settings.elementAmount).html(settings.amount.toString());$(settings.elementPrice).html("￥"+settings.price.toString());$(settings.elementSaving).html("￥"+settings.saving.toString());$(settings.elementBuy).attr("href",settings.addSkusUrl+wids.join(","))};accessory.init=function(){var _this=this,settings=this.settings;settings.fittingServUrl=settings.fittingServUrl.format(settings.skuid,settings.tokey);$.getJSON(settings.fittingServUrl,function(result){if(!result){return}var index=checkIndex(["tab-fitting","tab-group","placeholder-2a"]);settings.price=result.master.price;settings.saving=result.master.discount;if(typeof index=="number"){$(settings.elementTab).show();$(settings.elementTabcon).html(settings.template.process(result))}else{$(settings.elementTab).attr("load","true").addClass("curr").show();$(settings.elementTabcon).html(settings.template.process(result)).show();$(settings.element).show().jdTab({event:"click",index:0})}_this.setWidth($(settings.element),0)})};accessory.init();
/*suit*/
asyncScript(function(){var pshowskuid=$("#name").attr("pshowskuid");$.getJSONP("../../miaozhen/PromotionData.aspx@pid="+ pshowskuid +"&type=pack", getSuitService);});
...
function getSuitService(json){
	...
	var price=0,d1,src,html="";
		html1="<div class='p-img'><a href='{0}.html' target='_blank'><img {src}='{domain}n4/{1}' width='100' height='100' /></a></div><div class='p-name'><a href='{0}.html' target='_blank'>{2}</a></div>";
	if (data.length>1){
	...
	$.each(data,function(i){
		...
		html+="<div class='p-name'><a href='../suite/"+ data[i].PackId +"-"+ data[i].MainSkuId +".html' target='_blank'>"+ data[i].PackName +"</a></div>";
		...
		html+="<div class='btns'><a href='../../jd2008/purchase/initcart.aspx@pId="+ data[i].PackId +"&pCount=1&pType=2&mainSkuId="+ data[i].MainSkuId +"' class='btn-buy' clstag='shangpin|keycount|product|tzbuybtn'>购买套装</a></div>";
		html+="</div>";
		html+="</div>";	
	});	
...
/*gift*/
var GiftCardWid=null;function getGift(wid){$.jdThickBox({type:"json",width:600,height:530,title:"选择贺卡样式",source:"../../buy.360buy.com/GreetingCard/giftAjax.aspx@getGift=get&wid="+wid+"&jsoncallback=?"},function(url,ele){$.getJSON(url,function(json){if(json.info!=""){ele.html(json.info)}})})}function setGift(){var array=document.getElementById("changeCard").getElementsByTagName("input");var wid;for(var i=0;i<array.length;i++){if(array[i].type=='radio'&&array[i].checked){wid=array[i].value;break}}var to=document.getElementById("To").value;var form=document.getElementById("From").value;var msg=document.getElementById("Message").value;to=to.replace(/<[\s\S]*?>/,"");form=form.replace(/<[\s\S]*?>/,"");msg=msg.replace(/<[\s\S]*?>/,"");GiftCardWid=wid;if(wid==""||wid==null){alert("请选择贺卡样式！");return false}if(to==""||to==null){alert("请填写贺卡收件人！");return false}if(msg==""||msg==null){alert("请填写贺卡内容！");return false}if(msg.length>80){alert("贺卡内容长度不能超过80！目前已经输入"+msg.length);return false}if(form==""||form==null){alert("请填写贺卡发件人！");return false}$.getJSON("../../buy.360buy.com/GreetingCard/giftAjax.aspx@getGift=set&wid="+wid+"&To="+to+"&From="+form+"&msg="+msg+"&jsoncallback=?",function(json){if(json.info!=""){if(json.info=="true"){if(CardUrl!=null){window.location.href=CardUrl}else if(CardFun!=null){eval(CardFun)}}else{alert("保存信息失败！")}}})}var CardUrl=null;var CardFun=null;function Card(){this.Wid=0;this.Url='';this.Fun='';this.Show=function(){CardUrl=this.Url;CardFun=this.Fun;getGift(this.Wid)}}
...
var fq_serverSite = "../../jd2008/purchase/";
var fq_serverSiteService = "../../jd2008/purchaseservice/";
...
function fq_showFq_html(obj){
	if(obj!=null){
		if(obj.json.length==0){
			...
			fq_returnData=obj;
			//$.getJSONP("../SOAService/FqCMB.ashx@skuid="+fq_skuId+"&callback=fqMarketingService",fqMarketingService);
		}
	}
}
function fq_formatData(obj){
	...
	str+="<div class='Tip_Submit'><a  style='border:none' href='"+fq_serverSite+"ShoppingCart_fqInit.aspx@skuId="+fq_skuId+"&skuCount=1' id='Tip_apply'><img src='"+fq_serverSite+"skin/images/fqstart.gif' /></a><a class='link_1' style='margin-right:210px;border:none;color:#000' href='../../help/question-71.html' target='_blank'>查看分期付款帮助</a></div>";
	t.innerHTML=str;	
}
var buyBtnLink;
	...
	if(sDiv.length!=0||sDiv!=null){
		...
		if(obj!=null&&obj.json.length>0&&obj.json[0].error==null){
			...
			for(var i=0;i<obj.json.length;i++){
				...
				strYb+="<li><div class='content'>·<a target='_blank' href='../help/"+obj.json[i].HName+"'>"+obj.json[i].Name+"</a>";
				strYb+="<font color='red' style='margin-right:12px'>"+obj.json[i].Message+"</font><strong style='color:red'>￥"+obj.json[i].Price+"</strong>&nbsp;&nbsp;";
				strYb+="<a target='_blank' href='../help/"+obj.json[i].HName+"'>[详细说明]</a></div>";
			...
			for(var i=0;i<obj.json.length;i++){
				...
				strQx+="<li><div class='content'>·<a href='./"+obj.json[i].Id+".html' target='_blank'>"+obj.json[i].Name+"</a><font color='red' style='margin-right:12px'>"+obj.json[i].Message+"</font><strong style='color:red'>￥"+obj.json[i].Price+"</strong></div>";
				strQx+="<a href='"+fq_serverSite+"InitCart.aspx@pid="+obj.json[i].Id+"&pcount=1&ptype=1' class='btn-buy'>购买</a>";
				strQx+="&nbsp;&nbsp;<a href='../help/special/index.aspx' target='_blank'>[服务帮助]</a>";
				strQx+="</li>";
			}
			strQx+="</ul>";
			if(strQx!="<ul></ul>"){
			...
			for(var i=0;i<obj.json.length;i++){
				...
				strDiy+="<li><div class='content'>·<a href='./"+obj.json[i].Id+".html' target='_blank'>"+obj.json[i].Name+"</a><font color='red' style='margin-right:12px'>"+obj.json[i].Message+"</font><strong style='color:red'>￥"+obj.json[i].Price+"</strong></div>";
				strDiy+="<a href='"+fq_serverSite+"InitCart.aspx@pid="+obj.json[i].Id+"&pcount=1&ptype=1' class='btn-buy'>购买</a>";
				strDiy+="&nbsp;&nbsp;<a href='../help/special/index.aspx' target='_blank'>[服务帮助]</a>";
				strDiy+="</li>";
			}
			strDiy+="</ul>";
			if(strDiy!="<ul></ul>"){
...
function Card_buy_finish(mainSkuId){
	this.location='../../jd2008/purchase/InitCart.aspx@pid='+mainSkuId+'&pcount=1&ptype=1&addId='+GiftCardWid;
}
/*20110303*/
function refreshComments(referenceType,parameter){
    ...
    if(referenceId!=""){
        ...
        if(referenceType=="Product"){
            ...
            url="".format(referenceType,referenceId);
...
function GetSnsAvatar(){
	...
	if (pins.length > 0)
	{
		...
		$.getJSONP(
			"../../i.360buy.com/user/upload/img/show/" + encodeURI(escape(strPin)) + ".html@callback=GetAvatarCallback",
			GetAvatarCallback);
	}
}
...
var referServiceUrl="sayword?callback=?";
...
function refreshConsultations(pType){
    ...
    if(productId>0){
        $.getJSONP(""+productId+"&pType="+pType,getConsultations);
    }
}
/*20100601*/
