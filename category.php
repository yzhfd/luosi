...
if (!$smarty->is_cached('category.dwt', $cache_id)) {
    ...
    $goodslist1 = category_get_goods1($cat_id);
    ...
    $smarty->assign('goods_list1',$goodslist1);
    $smarty->assign('category', $cat_id);
    $smarty->assign('script_name', 'category');

    require(ROOT_PATH . 'themes/360buy/php/category.php'); //888
...
function category_get_goods1($cat_id) {
    $sql = 'SELECT g.*' .
    ...
    foreach ($res as $key => $row) {
        $sql = "SELECT goods_attr_id, attr_value FROM " . $GLOBALS['ecs']->table('goods_attr') . " WHERE goods_id = '$row[goods_id]'";
        ...
        $arr[$row['goods_id']]['attr_list']=$return_arr;
    }
