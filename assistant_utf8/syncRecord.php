...
function goodsImgList($goods_id){
	...
	$sql = "select img_original, img_url, thumb_url from ".$GLOBALS['ecs']->table('goods_gallery')." where goods_id=".$goods_id;	
	$rows = $GLOBALS['db']->GetAll($sql); 
	if ( $rows )
	{
		foreach ( $rows as $row )
...
function downloadGoods2(
		...
		)
	{
	    ...
	    $sql = "select goods_id, img_url,img_desc,thumb_url,img_original from ".$GLOBALS['ecs']->table('goods_gallery')." where goods_id=".$goods_id;	
		$sync_item['data'] = downloadRecord2($sql, $delimiter_rn, $delimiter_cn);	
		$syncitems[] = $sync_item;
